...
class Zo extends Jo {
  ...
  set(t, n, s, r) {
    ...
    const o = X(t) && Dr(n) ? Number(n) < t.length : ae(t, n),
...
function Ic(e, t, n = fe) {
  ...
  const E = (T) => {
    if (!(!(c.flags & 1) || (!c.dirty && !T)))
      if (t) {
        const O = c.run();
        if (r || g || (v ? O.some((G, j) => Pt(G, S[j])) : Pt(O, S))) {
          d && d();
          const G = Nt;
          Nt = c;
          try {
            const j = [O, S === Gn ? void 0 : v && S[0] === Gn ? [] : S, p];
            a ? a(t, 3, j) : t(...j), (S = O);
          } finally {
            Nt = G;
          }
...
function fu(e, t) {
  ...
  const {
    ...
    v = (
      ...
      const { type: L, ref: Y, shapeFlag: V } = m;
      switch (L) {
        case Ts:
          y(h, m, w, A);
          break;
        ...
        default:
          V & 1
            ...
            : V & 6
              ? B(h, m, w, A, R, P, F, N, D)
              : (V & 64 || V & 128) && L.process(h, m, w, A, R, P, F, N, D, $);
      }
    ...
    O = (h, m, w, A, R, P, F, N, D) => {
      m.type === "svg" ? (F = "svg") : m.type === "math" && (F = "mathml"),
        h == null ? G(m, w, A, R, P, F, N, D) : C(h, m, R, P, F, N, D);
    },
    G = (h, m, w, A, R, P, F, N) => {
      let D, L;
      const { props: Y, shapeFlag: V, transition: q, dirs: Q } = h;
      if (
        ((D = h.el = o(h.type, P, Y && Y.is, Y)),
        V & 8
          ? c(D, h.children)
          : V & 16 && M(h.children, D, null, A, R, zs(h, P), F, N),
        Q && Bt(h, null, A, "created"),
        j(D, h, h.scopeId, F, A),
        Y)
      ) {
        for (const pe in Y)
          pe !== "value" && !wn(pe) && i(D, pe, null, Y[pe], P, A);
        "value" in Y && i(D, "value", null, Y.value, P),
      ...
      se && q.beforeEnter(D),
        ...
        ((L = Y && Y.onVnodeMounted) || se || Q) &&
          je(() => {
            L && st(L, A, h), se && q.enter(D), Q && Bt(h, null, A, "mounted");
          }, R);
    },
    j = (h, m, w, A, R) => {
      ...
      if (R) {
        ...
        if (
          ...
          const F = R.vnode;
          j(h, F, F.scopeId, F.slotScopeIds, R.parent);
        }
      }
    },
    M = (h, m, w, A, R, P, F, N, D = 0) => {
      for (let L = D; L < h.length; L++) {
        const Y = (h[L] = N ? Tt(h[L]) : lt(h[L]));
        v(null, Y, m, w, A, R, P, F, N);
      }
    },
    C = (h, m, w, A, R, P, F) => {
      const N = (m.el = h.el);
      let { patchFlag: D, dynamicChildren: L, dirs: Y } = m;
      D |= h.patchFlag & 16;
      const V = h.props || fe,
        q = m.props || fe;
      let Q;
      if (
        ...
        w && Dt(w, !0),
        ((V.innerHTML && q.innerHTML == null) ||
          (V.textContent && q.textContent == null)) &&
          c(N, ""),
        L
          ? I(h.dynamicChildren, L, N, w, A, zs(m, R), P)
          : F || re(h, m, N, null, w, A, zs(m, R), P, !1),
        D > 0)
      ) {
        if (D & 16) H(N, V, q, w, R);
        else if (
          (D & 2 && V.class !== q.class && i(N, "class", null, q.class, R),
          D & 4 && i(N, "style", V.style, q.style, R),
          D & 8)
        ) {
          ...
          for (let pe = 0; pe < se.length; pe++) {
            const ce = se[pe],
              Fe = V[ce],
              Be = q[ce];
            (Be !== Fe || ce === "value") && i(N, ce, Fe, Be, R, w);
          }
        }
        D & 1 && h.children !== m.children && c(N, m.children);
      } else !F && L == null && H(N, V, q, w, R);
    ...
    I = (h, m, w, A, R, P, F) => {
      for (let N = 0; N < m.length; N++) {
        const D = h[N],
          L = m[N],
          Y =
            D.el && (D.type === Ee || !hn(D, L) || D.shapeFlag & 70)
              ? f(D.el)
              : w;
        v(D, L, Y, null, A, R, P, F, !0);
      }
    },
    H = (h, m, w, A, R) => {
      if (m !== w) {
    ...
    K = (h, m, w, A, R, P, F, N, D) => {
      ...
      let { patchFlag: V, dynamicChildren: q, slotScopeIds: Q } = m;
      Q && (N = N ? N.concat(Q) : Q),
        h == null
          ? (s(L, w, A), s(Y, w, A), M(m.children || [], w, Y, R, P, F, N, D))
          : V > 0 && V & 64 && q && h.dynamicChildren
            ? (I(h.dynamicChildren, q, w, R, P, F, N),
              (m.key != null || (R && m === R.subTree)) && Ll(h, m, !0))
            : re(h, m, w, Y, R, P, F, N, D);
    },
    B = (h, m, w, A, R, P, F, N, D) => {
      (m.slotScopeIds = N),
        h == null
          ? m.shapeFlag & 512
            ? R.ctx.activate(m, w, A, F, D)
            : U(m, w, A, R, P, F, D)
          : oe(h, m, D);
    },
    U = (h, m, w, A, R, P, F) => {
      const N = (h.component = Ru(h, A, R));
      if ((ml(h) && (N.ctx.renderer = $), Mu(N, !1, F), N.asyncDep)) {
        if ((R && R.registerDep(N, te, F), !h.el)) {
          const D = (N.subTree = de(Rt));
          b(null, D, m, w);
        }
      } else te(N, h, m, w, R, P, F);
    },
    oe = (h, m, w) => {
    ...
    te = (h, m, w, A, R, P, F) => {
      const N = () => {
        if (h.isMounted) {
          let { next: V, bu: q, u: Q, parent: se, vnode: pe } = h;
          {
            const tt = Bl(h);
            if (tt) {
              V && ((V.el = pe.el), Z(h, V, F)),
                tt.asyncDep.then(() => {
                  h.isUnmounted || N();
                });
              return;
          ...
          let ce = V,
            Fe;
          Dt(h, !1),
            V ? ((V.el = pe.el), Z(h, V, F)) : (V = pe),
            q && Kn(q),
            (Fe = V.props && V.props.onVnodeBeforeUpdate) && st(Fe, se, V, pe),
            Dt(h, !0);
          const Be = Ei(h),
            et = h.subTree;
          (h.subTree = Be),
          ...
          Dt(h, !1),
            se && Kn(se),
            !et && (V = Q && Q.onVnodeBeforeMount) && st(V, ce, m),
            Dt(h, !0);
          {
          ...
          if ((pe && je(pe, R), !et && (V = Q && Q.onVnodeMounted))) {
            const tt = m;
            je(() => st(V, ce, tt), R);
          }
          (m.shapeFlag & 256 ||
      ...
      h.scope.on();
      const D = (h.effect = new Uo(N));
      h.scope.off();
      const L = (h.update = D.run.bind(D)),
        Y = (h.job = D.runIfDirty.bind(D));
      (Y.i = h), (Y.id = h.uid), (D.scheduler = () => Gr(Y)), Dt(h, !0), L();
    },
    Z = (h, m, w) => {
      m.component = h;
    ...
    re = (h, m, w, A, R, P, F, N, D = !1) => {
      ...
      if (q > 0) {
        if (q & 128) {
          Ye(L, V, w, A, R, P, F, N, D);
          return;
        } else if (q & 256) {
          Ue(L, V, w, A, R, P, F, N, D);
          return;
        }
      }
      Q & 8
        ? (Y & 16 && $e(L, R, P), V !== L && c(w, V))
        : Y & 16
          ? Q & 16
            ? Ye(L, V, w, A, R, P, F, N, D)
            : $e(L, R, P, !0)
          : (Y & 8 && c(w, ""), Q & 16 && M(V, w, A, R, P, F, N, D));
    },
    Ue = (h, m, w, A, R, P, F, N, D) => {
      (h = h || Jt), (m = m || Jt);
      const L = h.length,
        Y = m.length,
        V = Math.min(L, Y);
      let q;
      for (q = 0; q < V; q++) {
        const Q = (m[q] = D ? Tt(m[q]) : lt(m[q]));
        v(h[q], Q, w, null, R, P, F, N, D);
      }
      L > Y ? $e(h, R, P, !0, !1, V) : M(m, w, A, R, P, F, N, D, V);
    },
    Ye = (h, m, w, A, R, P, F, N, D) => {
      let L = 0;
      const Y = m.length;
      let V = h.length - 1,
        q = Y - 1;
      for (; L <= V && L <= q; ) {
        const Q = h[L],
          se = (m[L] = D ? Tt(m[L]) : lt(m[L]));
        if (hn(Q, se)) v(Q, se, w, null, R, P, F, N, D);
        else break;
        L++;
      }
      for (; L <= V && L <= q; ) {
        const Q = h[V],
          se = (m[q] = D ? Tt(m[q]) : lt(m[q]));
        if (hn(Q, se)) v(Q, se, w, null, R, P, F, N, D);
        else break;
        V--, q--;
      }
      if (L > V) {
        if (L <= q) {
          const Q = q + 1,
            se = Q < Y ? m[Q].el : A;
          for (; L <= q; )
            v(null, (m[L] = D ? Tt(m[L]) : lt(m[L])), w, se, R, P, F, N, D),
              L++;
        }
      } else if (L > q) for (; L <= V; ) Te(h[L], R, P, !0), L++;
      else {
        const Q = L,
          se = L,
          pe = new Map();
        for (L = se; L <= q; L++) {
          const ke = (m[L] = D ? Tt(m[L]) : lt(m[L]));
        ...
        for (ce = li.length - 1, L = Be - 1; L >= 0; L--) {
          ...
          dn[L] === 0
            ? v(null, nt, w, ai, R, P, F, N, D)
            : et && (ce < 0 || L !== li[ce] ? Ge(nt, w, ai, 2) : ce--);
        }
      }
    },
    Ge = (h, m, w, A, R = null) => {
      const { el: P, type: F, transition: N, children: D, shapeFlag: L } = h;
      if (L & 6) {
      ...
      if (F === Ee) {
        s(P, m, w);
        for (let V = 0; V < D.length; V++) Ge(D[V], m, w, A);
        s(h.anchor, m, w);
        return;
      }
      if (F === Yn) {
      ...
      if (A !== 2 && L & 1 && N)
        if (A === 0) N.beforeEnter(P), s(P, m, w), je(() => N.enter(P), R);
        else {
          const { leave: V, delayLeave: q, afterLeave: Q } = N,
            se = () => s(P, m, w),
            pe = () => {
              V(P, () => {
                se(), Q && Q();
      ...
      if ((pe && (ce = F && F.onVnodeBeforeUnmount) && st(ce, m, h), Y & 6))
        ...
        se && Bt(h, null, m, "beforeUnmount"),
          Y & 64
            ...
            : L && !L.hasOnce && (P !== Ee || (V > 0 && V & 64))
              ? $e(L, m, w, !1, !0)
              : ((P === Ee && V & 384) || (!R && Y & 16)) && $e(D, m, w),
          A && $t(h);
      }
    ...
    zn = (h, m, w) => {
      const { bum: A, scope: R, job: P, subTree: F, um: N, m: D, a: L } = h;
      Si(D),
        Si(L),
        A && Kn(A),
        R.stop(),
...
function ql(e, t, n = {}, s, r, i) {
  ...
  const E = (M, C = "") => {
    ...
    O = Nn(T);
  s._s.set(e, O);
  const j = ((s._a && s._a.runWithContext) || df)(() =>
  ...
  function i(f, d, p) {
    ...
    for (const T of b) {
      ...
      if (
        ...
        const G = v.children;
        for (let j = 0; j < G.length; j++) i(G[j], S, p && p.children[j]);
      }
      p = p || S;
    }
    return E
...
function yd(e) {
  ...
  function b(_, z) {
    ...
    const $ = t.resolve(k, z),
      ue = _.hash || "";
    $.params = c(d($.params));
    const h = Df(s, le({}, _, { hash: Of(ue), path: $.path })),
      m = r.createHref(h);
    return le(
  ...
  function j(_, z) {
    ...
    let R;
    return (
      !h && Nf(s, $, k) && ((R = cn(16, { to: A, from: $ })), Ge($, $, !0, !1)),
    ...
    B ||
      (B = r.listen((_, z, k) => {
        ...
        const $ = b(_),
          ue = G($);
        if (ue) {
          j(le(ue, { replace: !0, force: !0 }), $).catch(Tn);
          return;
        }
        u = $;
        const h = a.value;
...
const Sh = vf("counter", () => {
    const e = ne(!1),
      ...
      E = ne(1);
    return {
      discoverMoviesLoading: e,
      playingMoviesLoading: n,
      popularMoviesLoading: r,
      upcomingMoviesLoading: o,
      topRatedMoviesLoading: a,
      movieLoading: c,
      trailerLoading: d,
      creditLoading: g,
      moreMoviesLoading: y,
      ...
      getMoviesBy: async (B = 1, U) => {
        try {
          ...
          (t.value = oe.data.results), (e.value = !1);
        } catch (oe) {
          alert(oe);
        }
      },
      getNowPlayingMovies: async (B = 1) => {
        try {
          ...
          (s.value = U.data.results), (n.value = !1);
        } catch (U) {
          alert(U);
        }
      },
      getPopularMovies: async (B = 1) => {
        try {
          ...
          (i.value = U.data.results), (r.value = !1);
        } catch (U) {
          alert(U);
        }
      },
      getUpcomingMovies: async (B = 1) => {
        try {
          ...
          (l.value = U.data.results), (o.value = !1);
        } catch (U) {
          alert(U);
        }
      },
      getTopRatedMovies: async (B = 1) => {
        try {
          ...
          (u.value = U.data.results), (a.value = !1);
        } catch (U) {
          alert(U);
        }
      },
      ...
      getVideoBy: async (B) => {
        try {
          ...
          (p.value = U.data.results), (d.value = !1);
        } catch (U) {
          alert(U);
        }
      },
      ...
      getMoreMovies: async (B = 1, U = "now_playing", oe = "") => {
        try {
          ...
          oe === ""
            ...
            (S.value = [...S.value, ...te.data.results]),
...
function $h() {
  ...
  if (
    ...
    j && e.grid.updateWrapperSize(M, d),
    !n.centeredSlides)
  ) {
    const I = [];
...
function fm(e, t, n, s, r) {
  ...
  if (l.normalizeSlideIndex)
    for (let j = 0; j < u.length; j += 1) {
      const M = -Math.floor(b * 100),
        C = Math.floor(u[j] * 100),
        I = Math.floor(u[j + 1] * 100);
      typeof u[j + 1] < "u"
        ? M >= C && M < I - (I - C) / 2
          ? (o = j)
          : M >= C && M < I && (o = j + 1)
  ...
  if (l.cssMode) {
    ...
    if (t === 0)
      ...
      if (!i.support.smoothScroll)
        return (
          La({ swiper: i, targetPosition: M, side: j ? "left" : "top" }), !0
        );
      p.scrollTo({ [j ? "left" : "top"]: M, behavior: "smooth" });
    }
    return !0;
  }
  const G = Na().isSafari;
...
function Fm() {
  ...
  d && !p
    ...
    ["navigation", "pagination", "scrollbar"].forEach((G) => {
      ...
      const j = s[G] && s[G].enabled,
        M = f[G] && f[G].enabled;
      j && !M && e[G].disable(), !j && M && e[G].enable();
    });
...
function Km(e) {
  ...
  r.includes("loop") &&
    t.isElement &&
    (c.loop && !s.loop ? (O = !0) : !c.loop && s.loop ? (G = !0) : (j = !0)),
    u.forEach((C) => {
      if (Ut(c[C]) && Ut(s[C]))
    ...
    r.includes("children") && n && c.loop && (j = !0),
    y && v.init() && v.update(!0),
  ...
  Ga = yd({
    ...
    routes: [
      { path: "/", name: "home", component: cg, meta: { title: "Home" } },
      {
        path: "/detail/:id",
        name: "detail",
        component: () => co(() => import("./DetailView-BaQwfaH7.js"), []),
        meta: { title: "Detail" },
      },
      {
        path: "/movie/:type",
        name: "movieList",
        component: () => co(() => import("./ListView-SABPZ6M5.js"), []),
        meta: { title: "List" },
      },
    ],
